...
namespace Microsoft.Samples.Kinect.HDFaceBasics
{
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged, IDisposable
    {
        ...
        public List<List<Double>> AP;
        public List<List<Double>> SP;
...
| FaceFrameFeatures.MouthOpen;
                    ...
                    if (this.faceFrameResult != null)
                    {
                        ...
                        else
                        {
                            if (EndBlink  == 1)
                            {
                                 if (PointsData != null)
                                 {
                                    AP = CalculateAccel(PointsData);
                                    SP = CalculateSpeed(PointsData);
                                    if (FrameBetFreq.Count >= 20)
                                    {
                                        freq = CalcFreq(FrameBetFreq);
                                        Sharrief.FreqOfBlinks = freq;
                                    }
                                    Sharrief.SpeedOfBlinks.Add(SP);
                                    Sharrief.AccelOfBlinks.Add(AP);
                                    if (Sharrief.SpeedOfBlinks.Count > 20)
                                    {
                                        Sharrief.SpeedOfBlinks.RemoveAt(0);
                                        TOPRINT += "Sharriefs's Biometric Data: Freq = " + Sharrief.FreqOfBlinks.ToString() + Environment.NewLine + "Speed: [";
                                        for (int i = 0; i < Sharrief.SpeedOfBlinks.Count; i++)
                                        {
                                            TOPRINT += "[ ";
                                            for (int j = 0; j < Sharrief.SpeedOfBlinks[0].Count; j++)
                                            {
                                                TOPRINT += "[ ";
                                                for (int n = 0; n < Sharrief.SpeedOfBlinks[0][0].Count; n++)
                                                {
                                                    TOPRINT += Sharrief.SpeedOfBlinks[i][j][n].ToString();
                                                    if (n != NewSubject.AccelOfBlinks[0][0].Count - 1)
                                                    {
                                                        TOPRINT += ", ";
                                                    }
                                                }
                                                TOPRINT += "]";
                                                if (n != NewSubject.AccelOfBlinks[0].Count - 1)
                                                {
                                                    TOPRINT += ", ";
                                                }
                                            }
                                            TOPRINT += "]";
                                            if (n != NewSubject.AccelOfBlinks.Count - 1)
                                            {
                                                TOPRINT += ", ";
                                            }
                                        }
                                    ...
                                    if (Sharrief.AccelOfBlinks.Count > 20)
                                    {
                                        Sharrief.AccelOfBlinks.RemoveAt(0);
                                        TOPRINT += "Accel: [";
                                        for (int i = 0; i < Sharrief.AccelOfBlinks.Count; i++)
                                        {
                                            TOPRINT += "[";
                                            for (int j = 0; j < Sharrief.AccelOfBlinks[0].Count; j++)
                                            {
                                                TOPRINT += "[ "; 
                                                for (int n = 0; n < Sharrief.AccelOfBlinks[0][0].Count; n++)
                                                {
                                                    TOPRINT += Sharrief.AccelOfBlinks[i][j][n].ToString();
                                                    if (n != NewSubject.AccelOfBlinks[0][0].Count - 1)
                                                    {
                                                        TOPRINT += ", ";
                                                    }
                                        ...
                                        TOPRINT += "]" + Environment.NewLine;
                                        Sharrief.AccelOfBlinks.Clear();
                                        Sharrief.SpeedOfBlinks.Clear();
                                        FrameBetFreq.Clear();
                                    }
                                }
                            }
        ...
        {
            var vertices = this.currentFaceModel.CalculateVerticesForAlignment(this.currentFaceAlignment);
            TOPRINT += "Vertices count = " + vertices.Count.ToString() + Environment.NewLine;
